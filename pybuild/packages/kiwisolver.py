...
class Kiwisolver(Package):
    source = GitSource('https://github.com/QPYPI/kiwisolver.git', alias='kiwi', branch='qpyc/1.0.1')
    patches = [
    ...
    def build(self):
        ...
        self.run([
            ...
            'setup.py',
            'build_ext',
            f'-I../../build/target/python/usr/include/python{PY_BRANCH}.{PY_M_BRANCH}:../../build/target/openblas/usr/include:{self.env["ANDROID_NDK"]}/sources/cxx-stl/gnu-libstdc++/4.9/include:{self.env["ANDROID_NDK"]}/sources/cxx-stl/gnu-libstdc++/4.9/libs/armeabi-v7a/include',

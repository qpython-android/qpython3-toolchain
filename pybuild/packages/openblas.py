...
class OpenBLAS(Package):
    ...
    def build(self):
        self.system(
            ...
            #f'make PREFIX={self.destdir()}/usr ARM_SOFTFP_ABI=1 TARGET=ARMV7 HOSTCC=gcc CC=arm-linux-androideabi-gcc FC=arm-linux-androideabi-gfortran FFLAGS=\"--sysroot {self.env["ARCH_SYSROOT"]}/..\" CFLAGS=\"--sysroot {self.env["ARCH_SYSROOT"]}/.. -D__ANDROID_API__=21 -I{self.env["ANDROID_NDK"]}/sysroot/usr/include/ -I{self.env["ANDROID_NDK"]}/sysroot/usr/include/arm-linux-androideabi/\" LDFLAGS=\"-lm -L{self.env["ANDROID_NDK"]}/toolchains/renderscript/prebuilt/linux-x86_64/platform/arm -lcompiler_rt -L{self.env["ARCH_SYSROOT"]}/lib -L{self.env["ANDROID_NDK_GFORTRAN"]}/toolchains/arm-linux-androideabi-4.9/prebuilt/linux-x86_64/arm-linux-androideabi/lib/armv7-a -lgfortran\" NETLIB_LAPACK_DIR=lapack-netlib'
            f'make PREFIX={self.destdir()}/usr ARM_SOFTFP_ABI=1 TARGET=ARMV7 AR=ar HOSTCC=gcc CC=\"clang -shared {self.env["CLANG_FLAGS_QPY"]}\" FC=arm-linux-androideabi-gfortran FFLAGS=\"--sysroot {self.env["ARCH_SYSROOT"]}/..\" CFLAGS=\"--sysroot {self.env["ARCH_SYSROOT"]}/.. -D__ANDROID_API__=21 -I{self.env["ANDROID_NDK"]}/sysroot/usr/include/ -I{self.env["ANDROID_NDK"]}/sysroot/usr/include/arm-linux-androideabi/\" LDFLAGS=\"-lm -L{self.env["ANDROID_NDK"]}/toolchains/renderscript/prebuilt/linux-x86_64/platform/arm -lcompiler_rt -L{self.env["ARCH_SYSROOT"]}/lib -L{self.env["ANDROID_NDK_GFORTRAN"]}/toolchains/arm-linux-androideabi-4.9/prebuilt/linux-x86_64/arm-linux-androideabi/lib/armv7-a -lgfortran\" NETLIB_LAPACK_DIR=lapack-netlib'
            )

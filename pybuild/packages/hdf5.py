...
class Hdf5(Package):
    source = GitSource('https://github.com/AIPYX/hdf5.git', alias='hdf5')
    patches = [
        #LocalPatch(''),
    ]
    ...
    def build(self):
        ...
        #self.system( f'if [ -e {self.destdir()}/lib/libzmq.so ] ; then mv {self.destdir()}/lib/libzmq.so {self.destdir()}/lib/libzmq.so.old; fi' )
